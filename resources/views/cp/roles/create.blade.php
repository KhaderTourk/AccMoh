...
<div class="w-full max-w-2xl space-y-6">
    ...
    <form action="{{ route('cp.roles.store') }}" method="post" class="rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 p-6 shadow-sm space-y-4">
        @csrf
        <div>
        ...
        <div>
            <label for="slug" class="cp-label block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">المعرّف (بالإنجليزية) *</label>
            <input type="text" name="slug" id="slug" value="{{ old('slug') }}" required class="cp-input w-full rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-2.5" placeholder="editor">
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="2" class="cp-input w-full rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-2.5">{{ old('description') }}</textarea>
        </div>
        <div>
                        ...
                        <div class="flex flex-wrap gap-3">
                            @foreach($items as $p)
                                <label class="inline-flex items-center gap-2">
                                    <input type="checkbox" name="permissions[]" value="{{ $p->id }}" {{ in_array($p->id, old('permissions', [])) ? 'checked' : '' }} class="rounded border-slate-300 text-primary">
