...
<div class="space-y-4">
    <form class="flex flex-wrap gap-2 items-end">
        <select name="fund_id" class="rounded-xl border px-3 py-2 dark:bg-slate-700"><option value="">الدرج</option>@foreach($funds as $f)<option value="{{ $f->id }}" @selected(request('fund_id')==$f->id)>{{ $f->name }}</option>@endforeach</select>
    ...
    <div class="rounded-2xl border bg-white dark:bg-slate-800 overflow-hidden">
        <table class="w-full text-sm text-right">
            <thead class="bg-slate-50 dark:bg-slate-700/50"><tr>
                <th class="px-3 py-2">التاريخ</th><th class="px-3 py-2">الوصف</th><th class="px-3 py-2">النوع</th><th class="px-3 py-2">الدرج</th><th class="px-3 py-2">الطريقة</th><th class="px-3 py-2">المبلغ</th>
            </tr></thead>
