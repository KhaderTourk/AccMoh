@extends('cp.layout')
@section('title', $take->exists ? 'تعديل منتج مأخوذ' : 'أخذ منتج من زبون')
@section('content')
@php
    $ilsId = (string) ($ilsCurrency?->id ?? '');
    $backUrl = route('cp.clients.show', $client);
@endphp
<form method="post"
      action="{{ $take->exists ? route('cp.client-goods-takes.update', $take) : route('cp.client-goods-takes.store') }}"
      class="max-w-2xl rounded-2xl border bg-white dark:bg-slate-800 p-6 space-y-4"
      x-data="goodsTakeForm()">
    @csrf
    @if($take->exists) @method('PUT') @endif
    <div>
        <h2 class="text-xl font-bold">{{ $take->exists ? 'تعديل منتج مأخوذ' : 'أخذ منتج من زبون' }}</h2>
        <p class="text-sm text-slate-500 mt-1">يُخصم سعر المنتج من المبلغ المستحق على الزبون دون تسجيل دفعة نقدية.</p>
    </div>
    <div>
        <label class="text-sm">الزبون *</label>
        <select name="client_id" required class="w-full rounded-xl border px-3 py-2 dark:bg-slate-700" @disabled($take->exists)>
            @foreach($clients as $c)
                <option value="{{ $c->id }}" @selected(old('client_id', $take->client_id)==$c->id)>{{ $c->personName() }}</option>
            @endforeach
        </select>
        @if($take->exists)<input type="hidden" name="client_id" value="{{ $take->client_id }}">@endif
    </div>
    <div>
        <label class="text-sm">اسم المنتج *</label>
        <input type="text" name="title" value="{{ old('title', $take->title) }}" required class="w-full rounded-xl border px-3 py-2 dark:bg-slate-700" placeholder="مثال: علبة شوكولاتة">
    </div>
    <div>
    ...
    <div>
        ...
        <input type="date" name="taken_on" value="{{ old('taken_on', optional($take->taken_on)->format('Y-m-d') ?? date('Y-m-d')) }}" required class="w-full rounded-xl border px-3 py-2 dark:bg-slate-700">
    </div>
    <div>
        <label class="text-sm">ملاحظات</label>
        <textarea name="notes" rows="2" class="w-full rounded-xl border px-3 py-2 dark:bg-slate-700">{{ old('notes', $take->notes) }}</textarea>
    </div>
...
@push('scripts')
<script>
function goodsTakeForm() {
    return {
        ilsId: @json($ilsId),
        currencyId: @json((string) old('currency_id', $take->fx_currency_id ?: $take->currency_id ?: $ilsId)),
        amount: @json((string) old('source_amount', old('amount', $take->source_amount ?: $take->amount))),
        rate: @json((string) old('exchange_rate', $take->formattedExchangeRate())),
