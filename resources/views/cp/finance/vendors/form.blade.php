...
<form method="post" action="{{ $vendor->exists ? route('cp.'.$type->routePrefix().'.update', $vendor) : route('cp.'.$type->routePrefix().'.store') }}" class="max-w-xl rounded-2xl border bg-white dark:bg-slate-800 p-6 space-y-4">
    ...
    <div class="flex items-center gap-2">
        ...
        <input type="checkbox" name="is_active" id="is_active" value="1" @checked(old('is_active', $vendor->is_active ? '1' : '0') == 1) class="rounded border-slate-300 text-primary">
...
@if($vendor->exists)
    <form method="post" action="{{ route('cp.'.$type->routePrefix().'.destroy', $vendor) }}" class="max-w-xl mt-3" onsubmit="return confirm('أرشفة {{ $type->label() }}؟ سيبقى ظاهراً في الجدول.')">
        @csrf @method('DELETE')
        <button type="submit" class="px-5 py-2 rounded-xl border border-rose-200 text-rose-600">أرشفة</button>
    </form>
@endif
@endsection
